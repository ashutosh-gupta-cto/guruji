...
import {
  createSampleBfsGraph,
  generateBfsSteps,
  type BfsData,
  type GraphEdge,
  type GraphNode,
} from '../visualizers/bfs';
...
export function BfsPage() {
  const [steps, setSteps] = useState<Step<BfsData>[]>([]);
  ...
  const run = useCallback(() => {
    const graph = createSampleBfsGraph();
    const next = generateBfsSteps(graph, 'A');
    setSteps(next);
  ...
  const description =
    steps[index]?.description ?? 'Run BFS to explore the graph layer by layer.';
  ...
  return (
    <div className="viz-panel">
      ...
      <svg viewBox={`0 0 ${SVG_W} ${SVG_H}`} className="viz-svg" role="img" aria-label="Breadth-first search">
        ...
        {current && current.visitOrder.length > 0 && (
          <text x={SVG_W - 20} y={18} fill="#10b981" fontSize={11} textAnchor="end">
            Order: {current.visitOrder.join(' → ')}
          </text>
        )}
